...
int main(int argc, char *argv[]){
    ...
    for(int i = 0; i < numLines; i++){
        ...
        // TODO: automate a way of ignoring whitespace.  
        ...
        if(strcmp(buffer[i], "Once_Upon_A_Time\n") == 0){ //f == i
            strcpy(buffer[i], "int main(int argc, char** argv){\n");
            //f = 1000;
        }
        if(strcmp(buffer[i], "The_End\n") == 0){ //e == i
            strcpy(buffer[i], "   return EXIT_SUCCESS;\n");
            //e = 2000;
        }
        ...
        if(strcmp(buffer[i], "    Shut\n") == 0){ //l == i
            strcpy(buffer[i], "   fclose(in_file);\n");
            //l = 1000;
        }
        if(strcmp(buffer[i], "Close_Book\n") == 0) {
            strcpy(buffer[i], "}"); 
        }
    }
    ...
    for(int i = 0; i < numLines; i++){
        ...
        // This was causing errors because the character pointers were pointing to string literals when they were replaced.  
        // Use strcpy instead.  
        free(buffer[i]);
    }
